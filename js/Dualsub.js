...
if (!service) _scriptSonverterDone({})
...
if (url.match(/action=get/)) {
    ...
    _scriptSonverterDone({ status: "HTTP/1.1 200 OK", body: JSON.stringify(setting), headers: { "Content-Type": "application/json" } })
}
...
if (url.match(/action=set/)) {
    ...
    _scriptSonverterDone({ status: "HTTP/1.1 200 OK", body: JSON.stringify(settings[service]), headers: { "Content-Type": "application/json" } })
}

let body = $response.body

if (service == "Netflix" && !body.match(/\d+:\d\d:\d\d.\d\d\d -->.+line.+\n.+/g)) _scriptSonverterDone({})

if (setting.type == "Disable") _scriptSonverterDone({})

if (setting.type != "Official" && url.match(/\.m3u8/)) _scriptSonverterDone({})

if (service == "YouTube") {

    let patt = new RegExp(`lang=${setting.tl}`)

    if (url.replace(/&lang=zh(-Hans)*&/, "&lang=zh-CN&").replace(/&lang=zh-Hant&/, "&lang=zh-TW&").match(patt) || url.match(/&tlang=/)) _scriptSonverterDone({})
    ...
    $task.fetch(options).then(response => {
        ...
        if (setting.line == "sl") _scriptSonverterDone({ body: response.body })
...
if (setting.type == "Official" && url.match(/\.m3u8/)) {
    ...
    if (!body.match(patt)) _scriptSonverterDone({})
}

if (url.match(/\.(web)?vtt/) || service == "Netflix" || service == "General") {
    if (service != "Netflix" && url == setting.s_subtitles_url && setting.subtitles != "null" && setting.subtitles_type == setting.type && setting.subtitles_sl == setting.sl && setting.subtitles_tl == setting.tl && setting.subtitles_line == setting.line) _scriptSonverterDone({ body: setting.subtitles })

    if (setting.type == "Official") {
        if (subtitles_urls_data == "null") _scriptSonverterDone({})
...
async function machine_subtitles(type) {
    ...
    if (!dialogue) _scriptSonverterDone({})
